...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      const keyPairAlice = new Keypair(); // key pair of alice generated in /src/keypair.js

      // Alice depositing 0.1 ether in L1
      // ********************************

      // creation of transaction by Alice
      const depositEther = utils.parseEther('0.1'); // convert eth into BigNumber instance of Wei
      const unspentDeposit = new Utxo({ amount: depositEther, keypair: keyPairAlice});
      ...
      // Alice withdrawing 0.08 ether in L2
      // **********************************

      const withdrawEther = utils.parseEther('0.08');

      // recepient address different from sender address i.e., Alice creating anonymity
      const recipient = '0x42EB768f2244C8811C63729A21A3569731535f06';
      const changedUnspentDeposit = new Utxo({ amount: depositEther.sub(withdrawEther), keypair: keyPairAlice});
  ...
  it('[assignment] iii. alice deposit 0.13 eth in L1 -> alice sends 0.06 eth in L2 -> Bob withdraw all his funds in L2', async () => {
      // [assignment] complete code here
      // instantiation of alice keypair and hardhat waffle
      const { tornadoPool, token, omniBridge } = await loadFixture(fixture);
